 ...
 */
class InquiriesResolvedCommand implements CommandInterface
{
...
/**
 * 这是一个获取多条已解决的咨询列表命令的处理类。
 *
 * @author shsrain <shsrain@163.com>
 */
class InquiriesResolvedCommandHandler extends AbstractCommandHandler
{
    ...
    {
        return 'InquiriesResolvedCommand';
    }
    ...
    public function inquiriesResolved()
    {
      ...
      foreach($results as $key=>$res)
      {
          ...
          $res2=$this->db_upihealth2->user()->where(array('username'=>$res['public_userid']))->fetch();
          if($res2 != null)
          {
            $result['data'][$key]['avatar'] = "http://api.didijiankang.cn/api/images/".$res2['avatar'];
          }
          else
          {
            $result['data'][$key]['avatar'] = null;
          }
      }
